...
def drivers():
    ...
    order = request.args.get('order', 'asc')
    sort_asc = order == 'asc'
    report_data = build_report(DATA_FOLDER).sort_values(by='abbr_driver', ascending=sort_asc)
    ...
    if driver_id:
        ...
        drivers_list = report_data[['abbr_driver','driver']].sort_values(by='driver', ascending=sort_asc)
